...
const vehicleType = computed(() => (
  types.value.find((item) => item.manufacturerVehicleTypeId === typeId.value) ?? null
))
const filteredItems = computed(() => {
  const normalizedKeyword = keyword.value.trim().toLocaleLowerCase('ja')
  return items.value.filter((item) => {
    const matchesKeyword = !normalizedKeyword
      || item.commandCode.toLocaleLowerCase('ja').includes(normalizedKeyword)
      || item.commandName.toLocaleLowerCase('ja').includes(normalizedKeyword)
      || (item.description ?? '').toLocaleLowerCase('ja').includes(normalizedKeyword)
    const matchesProcessType = processTypeFilter.value === 'ALL' || item.processType === processTypeFilter.value
    const matchesStatus = statusFilter.value === 'ALL'
      || (statusFilter.value === 'ACTIVE' && item.isActive)
      || (statusFilter.value === 'INACTIVE' && !item.isActive)
    return matchesKeyword && matchesProcessType && matchesStatus
  })
})
...
function processTypeLabel(value: string): string {
  return commandProcessTypeOptions.find((option) => option.value === value)?.label ?? value
}
...
async function load(): Promise<void> {
  ...
  try {
    const [manufacturerList, typeList, commandList] = await Promise.all([
      ...
      fetchTransportCommands(typeId.value, true),
    ])
    manufacturers.value = manufacturerList
    types.value = typeList
    items.value = commandList
  } catch (error) {
    pageError.value = message(error, 'コマンドの取得に失敗しました。')
  } finally {
    loading.value = false
  }
}
...
function open(item?: TransportCommand): void {
  ...
  Object.assign(form, {
    ...
    processType: item?.processType ?? 'MOVE',
...
async function save(): Promise<void> {
  if (!form.commandCode.trim() || !form.commandName.trim()) {
    formError.value = 'コマンドコードとコマンド名は必須です。'
    return
  }
  ...
  try {
    ...
    if (editingId.value) {
      await updateTransportCommand(editingId.value, request)
    } else {
      await createTransportCommand(typeId.value, request)
    }
    dialogVisible.value = false
    await load()
  } catch (error) {
    formError.value = message(error, 'コマンドの保存に失敗しました。')
  } finally {
    saving.value = false
  }
}
...
async function remove(item: TransportCommand): Promise<void> {
  ...
  pageError.value = null
  try {
    await deleteTransportCommand(item.commandId)
    await load()
  } catch (error) {
    pageError.value = message(error, 'コマンドの削除に失敗しました。')
  }
}
...
function backToTypes(): void {
  router.push({
    ...
    params: { manufacturerId: manufacturerId.value },
  })
}
  ...
  <MainLayout>
    <section class="transport-master-page">
      <nav class="transport-breadcrumbs" aria-label="パンくず">
        ...
        <RouterLink :to="{ name: 'transport-vehicle-types', params: { manufacturerId } }">
          {{ manufacturer?.name ?? 'メーカー' }}
        </RouterLink>
        <i class="pi pi-angle-right" />
      ...
      <section class="transport-hero">
        <div class="transport-hero__icon transport-hero__icon--command">
          <i class="pi pi-code" />
        </div>
      ...
      <section class="transport-panel">
        <div class="transport-toolbar transport-toolbar--commands">
          ...
          <Select
            ...
            option-label="label"
            option-value="value"
            class="transport-filter transport-filter--wide"
          />
          <Select
        ...
        <div class="transport-table transport-table--commands">
          ...
          >
            ...
            <div class="command-name-cell">
              ...
              <small>{{ item.description || '説明は登録されていません。' }}</small>
            </div>
            <span class="process-type-tag">
              <small>{{ item.processType }}</small>
              <strong>{{ processTypeLabel(item.processType) }}</strong>
            </span>
            ...
            <div class="transport-actions">
              <Button label="編集" icon="pi pi-pencil" text @click="open(item)" />
      ...
      <Dialog
        ...
        modal
        :header="editingId ? 'コマンドを編集' : 'コマンドを追加'"
      ...
      >
        <div class="transport-context-banner">
          ...
          <span>
            <small>登録先</small>
            <strong>{{ manufacturer?.name ?? 'メーカー' }} / {{ vehicleType?.vehicleType ?? '種別' }}</strong>
          </span>
        </div>

        <div class="transport-form transport-form--grid">
          <label>
            <span>コマンドコード <b>必須</b></span>
            <InputText v-model="form.commandCode" autofocus placeholder="例：TravelToPosture" />
          </label>
          <label>
            <span>コマンド名 <b>必須</b></span>
            <InputText v-model="form.commandName" placeholder="例：移動指示" />
          </label>
          <label class="transport-form__wide">
            <span>処理種別 <b>必須</b></span>
            <Select
              v-model="form.processType"
              :options="commandProcessTypeOptions"
              option-label="label"
              option-value="value"
            />
          </label>
          <label class="transport-form__wide">
            <span>説明</span>
            <Textarea v-model="form.description" rows="4" auto-resize placeholder="APIの用途や補足情報" />
          </label>
          <label>
            <span>表示順</span>
          ...
          <p v-if="formError" class="transport-message transport-message--error transport-form__wide" role="alert">
            <i class="pi pi-exclamation-circle" />
            {{ formError }}
          </p>
        </div>
